...
UnitTest.test('TableAdjustmentsTest', () => {
  ...
  const testAdjustWidth = (msg: string, expectedWidth: number, expectedColumnWidths: number[], table: Element<HTMLTableElement>, step: number, index: number, direction: ResizeDirection, columnSizing: ColumnResizing) => {
    ...
    // Verify that the differnce is less than 1% to allow for minor floating point differences
  ...
  const testInnerColumnResizing = () => {
    ...
    testAdjustWidth(`ltr large step (%) - resiztable (0)`, 75, [ 50, 16.67, 16.67, 16.67 ], relativeTable(), percentageToStep(50, 400), 0, ResizeDirection.ltr, 'resizetable');
    testAdjustWidth(`ltr large step (%) - resiztable (1)`, 75, [ 16.67, 50, 16.67, 16.67 ], relativeTable(), percentageToStep(50, 400), 1, ResizeDirection.ltr, 'resizetable');
    testAdjustWidth(`ltr large step (%) - resiztable (2)`, 75, [ 16.67, 16.67, 50, 16.67 ], relativeTable(), percentageToStep(50, 400), 2, ResizeDirection.ltr, 'resizetable');
  ...
  const testLastColumnResizing = () => {
    ...
    testAdjustWidth(`ltr step (%) - default (3)`, 56.25, [ 25, 25, 25, 25 ], relativeTable(), percentageToStep(12.5, 400), 3, ResizeDirection.ltr, 'default');
