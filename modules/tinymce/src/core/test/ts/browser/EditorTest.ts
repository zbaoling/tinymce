...
UnitTest.asynctest('browser.tinymce.core.EditorTest', (success, failure) => {
  ...
  const sEventChange = (editor: Editor) => Log.step('', 'Event: change', Step.sync(() => {
    let level, lastLevel;
  ...
  const sEventBeforeExecCommand = (editor: Editor) => Log.step('', 'Event: beforeExecCommand', Step.sync(() => {
    let cmd, ui, value;
  ...
  const sRelativeUrls = (editor: Editor) => Log.step('', 'urls - relativeURLs', Step.sync(() => {
  ...
  const sAbsoluteUrls = (editor: Editor) => Log.step('', 'urls - absoluteURLs', Step.sync(() => {
  ...
  const sWebkidSerializationRangeBug = (editor: Editor) => Log.step('', 'WebKit Serialization range bug', Step.sync(() => {
    if (Env.webkit) {
  ...
  const sEditorMethodsGetParam = (editor: Editor) => Log.step('', 'editor_methods - getParam', Step.sync(() => {
  ...
  const sSetContent = (editor: Editor) => Log.step('', 'setContent', Step.sync(() => {
    let count;
  ...
  const sSetContentWithCommentBug = (editor: Editor) => Log.step('', 'setContent with comment bug #4409', Step.sync(() => {
  ...
  const sCustomElements = (editor: Editor) => Log.step('', 'custom elements', Step.sync(() => {
  ...
  const sStoreRestoreTabindex = (editor: Editor) => Log.step('', 'Store/restore tabindex', Step.sync(() => {
  ...
  const sShowHideIsHiddenAndEvents = (editor: Editor) => Log.step('', 'show/hide/isHidden and events', Step.sync(() => {
    let lastEvent;
  ...
  const sHideSaveContentAndHiddenStateWhileSaving = (editor: Editor) => Log.step('', 'hide save content and hidden state while saving', Step.sync(() => {
  ...
  const sAddCommand = (editor: Editor) => Log.step('', 'addCommand', Step.sync(() => {
    const scope = {};
  ...
  const sAddQuertyStateHandler = (editor: Editor) => Log.step('', 'addQueryStateHandler', Step.sync(() => {
    const scope = {};
  ...
  const sBlockScriptExecution = (editor: Editor) => Log.step('', 'Block script execution', Step.sync(() => {
  ...
  const sAddQueryValueHandler = (editor: Editor) => Log.step('', 'addQueryValueHandler', Step.sync(() => {
    const scope = {};
  ...
  const sSetDirtyIsDirty = (editor: Editor) => Log.step('', 'setDirty/isDirty', Step.sync(() => {
    let lastArgs = null;
  ...
  const sSetMode = (editor: Editor) => Log.step('', 'setMode', Step.sync(() => {
    let clickCount = 0;
  ...
  const sTranslate = (editor: Editor) => Log.step('', 'translate', Step.sync(() => {
  ...
  const sTreatSomeParagraphsAsEmptyContents = (editor: Editor) => Log.step('', 'Treat some paragraphs as empty contents', Step.sync(() => {
  ...
  const sKamerWordBoundaries = (editor: Editor) => Log.step('', 'kamer word boundaries', Step.sync(() => {
    editor.setContent('<p>!\u200b!\u200b!</p>');
    Assert.eq('kamer word boundaries', '<p>!\u200b!\u200b!</p>', editor.getContent());
  }));

  const sPreserveWhitespacePreElements = (editor: Editor) => Log.step('', 'Preserve whitespace pre elements', Step.sync(() => {
    editor.setContent('<pre> </pre>');
    Assert.eq('kamer word boundaries', '<pre> </pre>', editor.getContent());
  }));

  const sHasFocus = (editor: Editor) => Log.step('', 'hasFocus', Step.sync(() => {
    editor.focus();
