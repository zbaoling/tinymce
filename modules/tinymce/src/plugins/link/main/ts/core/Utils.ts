...
const link = (editor: Editor, attachState: AttachState, data: LinkDialogOutput) => {
  editor.hasPlugin('rtc') ? editor.execCommand('createlink', false, unwrapOptions(data)) : linkDomMutation(editor, attachState, data);
};

const unlink = (editor: Editor) => {
  editor.hasPlugin('rtc') ? editor.execCommand('unlink') : unlinkDomMutation(editor);
};
